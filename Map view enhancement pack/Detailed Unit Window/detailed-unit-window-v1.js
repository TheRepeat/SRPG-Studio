	...
	MapParts.UnitInfoLarge = defineObject(MapParts.UnitInfo,
		{
			...
			_drawSubInfo: function (x, y, unit, textui, mhp) {
				...
				if (ItemControl.getEquippedWeapon(unit) === null) {
					...
					TextRenderer.drawKeywordText(x + dx1[1], y, StringTable.SignWord_WaveDash, -1, color, font);
				...
				// equipped weapon
				ItemRenderer.drawItem(x, y, weapon, textui.getColor(), textui.getFont(), false);
				y += 22;
				...
				TextRenderer.drawText(x + dx1[2], y + 3, CRIT_AVOID_STAT, 64, statColor, font);
				NumberRenderer.drawNumber(x + dx1[3], y, cav);
				// TextRenderer.drawSignText(x + dx1[2], y, root.queryCommand('wlv_param'));
				// NumberRenderer.drawNumber(x + dx1[3], y, wlv);
				TextRenderer.drawText(x + dx1[6], y + 3, root.queryCommand('avoid_capacity'), 64, statColor, font);
				NumberRenderer.drawNumber(x + dx1[7], y, avo);
			},
