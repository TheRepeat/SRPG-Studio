...
(function () {
    ...
    MapLayer._drawIcons = function() {
        ...
        for (var i = 0; i < UnitStateAnimator._iconArray.length; i++) {
            ...
            if (!hide) GraphicsRenderer.drawImage(icon.x - session.getScrollPixelX(), icon.y - session.getScrollPixelY() - dy, icon.handle[icon.cycle], GraphicsType.ICON);
        }
    }

    // Data structure for the iconArray elements
    StructureBuilder.buildIconRenderInfo = function(x, y, handle) {
        var info = {
            x: 0,
            y: 0,
            handle: [],
...
var UnitStateAnimator = {
    ...
    addIconXY: function(x, y, handle) {
        if (!handle.isNullHandle()) {
            if (this._icon == null) {
                this._icon = StructureBuilder.buildIconRenderInfo(x, y, handle);
            } else {
    ...
    _addIcon: function(unit, handle) {
        var x = unit.getMapX() * GraphicsFormat.MAPCHIP_WIDTH + GraphicsFormat.MAPCHIP_WIDTH/8;
        var y = unit.getMapY() * GraphicsFormat.MAPCHIP_HEIGHT - GraphicsFormat.MAPCHIP_HEIGHT/2;

        this.addIconXY(x, y, handle);
    },
