...
3. Copy the following command into the Code box: JingleControl.resetSingleUnit(unitId);
 ...
 (B) Select Speech
Select Speech is a little trickier, but if you know how to use Execute Script event commands (and especially if you know the most basic ability to call a function in javascript), you're golden.
...
5. Escape every single-quote with a backslash. (If there is a crash, the most likely culprit is that you forgot to escape a quote.)
6. Now, add all of the control characters you want. Remember to use double slashes.
7. When you're happy with it, copy all of your new text and paste it in the same document. Now, put all of it on one line. It'll look ugly, I know. (It's important you keep a copy of the formatted, multi-line text in case you need to edit it later.)
